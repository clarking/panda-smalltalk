...
void st_unicode_canonical_decomposition(const uint32_t *in, int inlen, uint32_t **out, int *outlen);
...
uint32_t st_utf8_get_unichar(const char *p);

bool st_utf8_validate(const char *string, ssize_t max_len);

int st_unichar_to_utf8(uint32_t ch, char *outbuf);

const char *st_utf8_offset_to_pointer(const char *string, uint offset);

uint32_t *st_utf8_to_ucs4(const char *string);
