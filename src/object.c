...
void st_object_set_format(Oop object, st_format format) {
	(((((ObjHeader *) st_detag_pointer(object))->mark)) = ((((((ObjHeader *) st_detag_pointer(object))->mark)) & ~(ST_OBJECT_FORMAT_MASK << ST_OBJECT_FORMAT_SHIFT)) | (((format) & ST_OBJECT_FORMAT_MASK) << ST_OBJECT_FORMAT_SHIFT)));
}

st_format st_object_format(Oop object) {
	return ((((((ObjHeader *) st_detag_pointer(object))->mark)) >> ST_OBJECT_FORMAT_SHIFT) & ST_OBJECT_FORMAT_MASK);
}

void st_object_set_hashed(Oop object, bool hashed) {
	(((((ObjHeader *) st_detag_pointer(object))->mark)) = ((((((ObjHeader *) st_detag_pointer(object))->mark)) & ~(ST_OBJECT_HASH_MASK << ST_OBJECT_HASH_SHIFT)) | (((hashed) & ST_OBJECT_HASH_MASK) << ST_OBJECT_HASH_SHIFT)));
}

bool st_object_is_hashed(Oop object) {
	return ((((((ObjHeader *) st_detag_pointer(object))->mark)) >> ST_OBJECT_HASH_SHIFT) & ST_OBJECT_HASH_MASK);
}

uint st_object_instance_size(Oop object) {
	return ((((((ObjHeader *) st_detag_pointer(object))->mark)) >> ST_OBJECT_SIZE_SHIFT) & ST_OBJECT_SIZE_MASK);
}

uint st_object_set_instance_size(Oop object, uint size) {
	(((((ObjHeader *) st_detag_pointer(object))->mark)) = ((((((ObjHeader *) st_detag_pointer(object))->mark)) & ~(ST_OBJECT_SIZE_MASK << ST_OBJECT_SIZE_SHIFT)) | (((size) & ST_OBJECT_SIZE_MASK) << ST_OBJECT_SIZE_SHIFT)));
}
